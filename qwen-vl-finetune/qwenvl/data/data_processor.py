...
def _make_abs_paths(base: Path, files: Any) -> Any:
    if isinstance(files, (list, tuple)):
        return [_make_abs_paths(base, item) for item in files]
...
def resolve_video_fps_list(
    metadata: Dict[str, Any] | None,
    default_fps: float,
    n_videos: int,
) -> list[float]:
    """Resolve effective fps for already-sampled frame-list videos."""
...
def _is_frame_list_video(video: Any) -> bool:
    return (
        isinstance(video, list)
        and len(video) > 0
        and all(isinstance(frame, str) and frame for frame in video)
    )


def _video_processor_fps(processor: Any, default: float = 2.0) -> float:
    video_processor = getattr(processor, "video_processor", None)
    fps = _float_or_none(getattr(video_processor, "fps", None))
    return fps if fps is not None else default


def _frame_list_processor_kwargs(
    item: Dict[str, Any],
    processor: Any,
) -> Dict[str, Any]:
    ...
    if not isinstance(videos, list) or not videos:
        return {}
    if not all(_is_frame_list_video(video) for video in videos):
        return {}

    fps_list = resolve_video_fps_list(
        item.get("metadata") or {},
        _video_processor_fps(processor),
        len(videos),
    )
    video_metadata = []
    for video, fps in zip(videos, fps_list):
    ...
    return {"do_sample_frames": False, "video_metadata": video_metadata}
...
def preprocess_qwen_visual(
    ...
    processor_kwargs = _frame_list_processor_kwargs(source, processor)
