...
# Single-GPU Qwen3-VL-4B LoRA SFT for the final VideoProxy mixed Proxy data:
# base TG/MCQ + HierSeg + Event Logic + Temporal AoT.
...
DATASET_USE="${DATASET_USE:-videoproxy_proxy_mix_sft}"

SFT_MODEL_ROOT="${SFT_MODEL_ROOT:-/m2v_intern/xuboshen/zgw/SFT-Models/VideoProxyMixed}"
EXP_NAME="${EXP_NAME:-qwen3vl4b_videoproxy_proxy_mix_sft_lora_1ep_1gpu_256f_tok8192_len16k}"
...
echo "[videoproxy-proxy-mix-sft] model: ${MODEL_PATH}"
echo "[videoproxy-proxy-mix-sft] dataset: ${DATASET_USE}"
echo "[videoproxy-proxy-mix-sft] output: ${OUTPUT_DIR}"
echo "[videoproxy-proxy-mix-sft] tensorboard: ${TENSORBOARD_DIR}"
echo "[videoproxy-proxy-mix-sft] gpus: CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}, nproc=${NPROC_PER_NODE}"
echo "[videoproxy-proxy-mix-sft] batch: micro=${BATCH_SIZE}, grad_accum=${GRAD_ACCUM_STEPS}, effective=${EFFECTIVE_BATCH}"
echo "[videoproxy-proxy-mix-sft] video: frames=${VIDEO_MAX_FRAMES}, tokens=${VIDEO_MAX_TOKENS}, pixels=${VIDEO_MAX_PIXELS}, fps=${VIDEO_FPS}"
echo "[videoproxy-proxy-mix-sft] max_length: ${MODEL_MAX_LENGTH}"
