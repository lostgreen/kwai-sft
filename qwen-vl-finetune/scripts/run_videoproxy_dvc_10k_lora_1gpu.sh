...
# Single-GPU Qwen3-VL-4B LoRA SFT for VideoProxy Dense Video Caption data.
# This task predicts one timestamped L2 event list per full video, so the
# default text context is larger than the whole-video description run.
...
DATASET_USE="${DATASET_USE:-videoproxy_dvc_10k}"

SFT_MODEL_ROOT="${SFT_MODEL_ROOT:-/m2v_intern/xuboshen/zgw/SFT-Models/VideoProxyMixed}"
EXP_NAME="${EXP_NAME:-qwen3vl4b_videoproxy_dvc10k_lora_2ep_1gpu_256f_tok8192_len16k}"
...
echo "[videoproxy-dvc-10k] model: ${MODEL_PATH}"
echo "[videoproxy-dvc-10k] dataset: ${DATASET_USE}"
echo "[videoproxy-dvc-10k] output: ${OUTPUT_DIR}"
echo "[videoproxy-dvc-10k] tensorboard: ${TENSORBOARD_DIR}"
echo "[videoproxy-dvc-10k] gpus: CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}, nproc=${NPROC_PER_NODE}"
echo "[videoproxy-dvc-10k] batch: micro=${BATCH_SIZE}, grad_accum=${GRAD_ACCUM_STEPS}, effective=${EFFECTIVE_BATCH}"
echo "[videoproxy-dvc-10k] video: frames=${VIDEO_MAX_FRAMES}, tokens=${VIDEO_MAX_TOKENS}, pixels=${VIDEO_MAX_PIXELS}, fps=${VIDEO_FPS}"
echo "[videoproxy-dvc-10k] max_length: ${MODEL_MAX_LENGTH}"
